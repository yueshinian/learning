...
class DualResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        layers = []
        #1
        x = self.conv1(x)#2个3×3卷积，stride=2, resolution/4
        #2 high
        x = self.layer1(x)
        layers.append(x)
        #2 high resolution/8
        ...
        layers.append(x)
        #3
        x = self.layer3(self.relu(x))#low，resolution/16
        layers.append(x)
        x_ = self.layer3_(self.relu(layers[1]))#high 1/8

        x = x + self.down3(self.relu(x_))#low
        x_ = x_ + F.interpolate(
        ...
        if self.augment:
            temp = x_#seghead
        #4
        x = self.layer4(self.relu(x))#low resolution/32
        layers.append(x)
        x_ = self.layer4_(self.relu(x_))#high 1/8

        x = x + self.down4(self.relu(x_))#low
        x_ = x_ + F.interpolate(
                        self.compression4(self.relu(layers[3])),
                        size=[height_output, width_output],
                        mode='bilinear')#high
        #5
        x_ = self.layer5_(self.relu(x_))#高分辨率分支（主分支）high
        x = F.interpolate(
                        self.spp(self.layer5(self.relu(x))),
                        size=[height_output, width_output],
                        mode='bilinear')#低分辨率分支low
        #sum
        x_ = self.final_layer(x + x_)#组合不同尺度的特征，channels_num to classes_num，3×3卷积分+ 1×1卷积
